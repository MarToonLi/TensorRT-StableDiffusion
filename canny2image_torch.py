...
class hackathon():
    ...
    def process(self, input_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, ddim_steps, guess_mode,
        ...
        with torch.no_grad():
            ...
            detected_map = self.apply_canny(img, low_threshold, high_threshold)     # 0 和 255
            detected_map = HWC3(detected_map)

            control = torch.from_numpy(detected_map.copy()).float().cuda() / 255.0  # 归一化
            ...
            if config.save_memory:  # 部分模型的计算在CPU上，部分在GPU上
